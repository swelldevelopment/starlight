...
class Router
{
    ...
    protected function _filter($match, $filter, $response = null)
    {
        ...
        if ($match['controller']) {
            ...
            if (method_exists($obj, 'filter_' . $filter) && is_callable([$obj, 'filter_' . $filter])) {
                $this->_callFilter([$obj, 'filter_' . $filter], $args);
            }
            ...
            if (method_exists($obj, 'filter') && is_callable([$obj, 'filter'])) {
                $this->_callFilter([$obj, 'filter'], $args);
            }
        }
    ...
    {
        // !!! Should not need this anymore (8/1/2025)
        //if (PHP_VERSION >= 8) {
    ...
    protected function _matchObject($clss, &$match)
    {
        ...
        $match['objects'][$objKey] = isset($match['objects'][$objKey]) ? $match['objects'][$objKey] : new $clss($this, $match);
